...
export default async function PayHereReturnPage({
  ...
  if (!session?.user) {
    redirect(`/login?callbackUrl=${encodeURIComponent(`/bookings/${id}/pay/return`)}`);
  }
      ...
      <Container className="max-w-xl py-12 md:py-16">
        <div className="space-y-6 rounded-2xl border border-[#DFE1E4] bg-white p-6 shadow-sm md:p-8">
          ...
          <p className="text-sm text-[#6B7280]">
            If the payment succeeded, your booking will show as paid once PayHere
            confirms it (usually within a few seconds). You can refresh the booking
            page if status has not updated yet.
          </p>
          <div className="flex flex-wrap gap-3">
            ...
            <Button render={<Link href={`/bookings/${id}/pay`} />} variant="outline">
              Back to checkout
            </Button>
          </div>
        </div>
